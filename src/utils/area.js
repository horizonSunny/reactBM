...
export function newArea() {
  ...
  Area.forEach(item => {
    let tempData = {};
    tempData.value = item.provinceCode;
    tempData.label = item.provinceName;
    if (item.mallCityList.length > 0) {
      tempData.children = [];
      item.mallCityList.forEach(ele => {
        let tempCity = {};
        tempCity.value = ele.cityCode;
        tempCity.label = ele.cityName;
        if (ele.mallAreaList.length > 0) {
          tempCity.children = [];
          ele.mallAreaList.forEach(res => {
            let tempArea = {};
            tempArea.value = res.areaCode;
            tempArea.label = res.areaName;
